...
module Operator
  def flow(input_queue, output_queue, methods, klass, dependencies)
  ...
  def source(input_queue, output_queue, method, klass, dependencies)
    ctx = klass.new(dependencies)
...
class PipeDSL
  ...
  def source(method_name)
    ...
    @reactor.async do
      super(input_queue, output_queue, method_name, @klass, @initializers)
  ...
  def flow(methods)
    ...
    @reactor.async do
      super(input_queue, output_queue, methods, @klass, @initializers)
    end
  end
